...
| T-P3-006 | Backend regression suite | Regression | `docker compose run --rm backend pytest` | ✅ Done | Backend suite passed after scheduled scan follow-up: 148 tests |
...
| T-P3-012 | Scheduled scan runner tests | Unit/API | `docker compose run --rm backend pytest tests/test_scheduled_scan.py tests/test_scanner_config.py tests/test_display_settings.py` | ✅ Done | 32 targeted tests passed; covers schedule timing, local startup catch-up, blank optional env parsing, config validation, and settings schedule copy |
