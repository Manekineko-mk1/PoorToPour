...
| D-042 | 2026-04-30 | 🟧 Replaced | Defer final market data provider selection until implementation testing | Provider pricing, rate limits, adjusted data quality, and endpoint behavior need hands-on validation | Requires a provider experiment step before full implementation | Replaced by D-070, then superseded by D-077: use yfinance pragmatically for MVP scan data while retaining Alpha Vantage as an adapter |
...
| D-044 | 2026-04-30 | 🟧 Replaced | MVP data sourcing will use Tier 1 provider-backed data only for core scanner inputs | Keeps MVP implementation simple, reliable, and easier to validate before adding hybrid data complexity | Tier 2 official public APIs and Tier 3 scraping are delayed | Replaced by D-077: yfinance is acceptable as the pragmatic MVP scan-data source while preserving source/freshness labelling and provider abstraction |
...
| D-065 | 2026-05-25 | 🟧 Replaced | Use yfinance as a Phase 1 OHLCV bootstrap adapter only | It lets the project validate ingestion, normalization, and storage with real daily bars before choosing a paid/provider-backed source | yfinance is unofficial and should not be treated as final trading-grade data | Replaced by D-077: use yfinance pragmatically as the MVP scan data source while scanner/strategy value is being proven |
...
| D-070 | 2026-05-26 | 🟧 Replaced | Use Alpha Vantage as the first real market-data provider adapter | Jesse already has an Alpha Vantage key, and it is good enough to test the provider abstraction after yfinance bootstrap | API limits may constrain full-universe refreshes and require batching, caching, and rate-limit handling | Replaced by D-077 for MVP scan-data default; Alpha Vantage remains implemented and available as a future stable-source candidate |
...
| D-077 | 2026-05-28 | 🟩 Accepted | Use yfinance pragmatically as the MVP scan data source while retaining Alpha Vantage as an adapter and future stable-source candidate | Alpha Vantage's free call limit is too restrictive for S&P 500 plus Nasdaq 100 iteration, while yfinance is more attractive for a cost-constrained side-project MVP | yfinance is unofficial and not trading-grade; data freshness/source must be visible and the provider abstraction must stay intact | Lets Phase 3 prove the dashboard and scanner workflow cheaply before paying for or committing to a more stable data provider |
| D-078 | 2026-05-28 | 🟩 Accepted | Use TradingView Lightweight Charts for Phase 3 candidate chart evidence | It aligns with the desired trading-dashboard feel and should support candlesticks, volume, indicators, and overlays without excessive custom chart work | Adds a frontend chart dependency and integration work | Defines the first chart implementation path for Dashboard MVP |
| D-079 | 2026-05-28 | 🟩 Accepted | Candidate Detail should be a real route for MVP traceability while the dashboard may keep a selected-candidate preview | A durable route is easier to inspect, share, test, and link from scan history | Slightly more routing/state work than a single-page panel only | Clarifies Phase 3 navigation and candidate-review workflow |
| D-080 | 2026-05-28 | 🟩 Accepted | Phase 3 Settings should start with read-only system/admin options plus safe user preferences | MVP is single-user, but the UI should not normalize arbitrary user edits to provider, secret, or system scan settings | Some scan configuration remains code/env driven for now | Keeps settings useful without introducing unsafe or premature configuration persistence |
...
| D-044 | 2026-05-28 | D-077 | Tier 1-only core scanner data policy relaxed for cost-constrained MVP iteration with yfinance |
| D-065 | 2026-05-28 | D-077 | yfinance moved from Phase 1 bootstrap-only to pragmatic MVP scan-data source while scanner value is being proven |
| D-070 | 2026-05-28 | D-077 | Alpha Vantage remains available as an adapter, but it is no longer the default full-universe MVP scan data source |
...
| 2026-05-28 | Added Phase 3 decisions for yfinance-first MVP scan data, TradingView Lightweight Charts, candidate detail routing, and settings scope | Jesse + AI |
